...
document.addEventListener('DOMContentLoaded', () => {
  ...
  window.alert=(message)=>{
    if(message==='Sell date must be after buy date.'){
      const bd=document.getElementById('mBD'),sd=document.getElementById('mSD');
      if(bd&&sd&&bd.value&&sd.value&&sd.value===bd.value){
        (async()=>{
          try{
            ...
            if(!apps.length)throw new Error('Firebase is not ready');
            const auth=authMod.getAuth(apps[0]),user=auth.currentUser;
            ...
            if(!ticker||!(q>0)||!(b>0)||!(s>=0)||!bd.value||!sd.value){nativeAlert('Please complete all fields.');return}
            ...
            const db=fs.getFirestore(apps[0]);
          ...
          }catch(e){console.error(e);nativeAlert('Could not save trade.');}
        })();
        return;
      }
    }
  ...
  const apply=()=>{applyPuppyLogo();applyLoginMessage();addHeaderControls();addChangePassword();addPasswordRecovery()};apply();new MutationObserver(apply).observe(document.body,{childList:true,subtree:true});
});
