...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const removeOldAccountText=()=>{document.querySelectorAll('p,div,span').forEach(el=>{if(el.children.length===0&&el.textContent.includes("Use this same username and password on your father's phone.")){el.textContent='Manage your account and sign out securely.'}})};
  const forceAccountMessage=()=>{
    const fix=()=>{
      document.querySelectorAll('.modal').forEach(modal=>{
        const h=modal.querySelector('h3');
        const p=modal.querySelector('p');
        if(h&&h.textContent.trim()==='Account'&&p){
          p.textContent='Manage your account and sign out securely.';
        }
      });
      removeOldAccountText();
    };
    fix();
    [0,25,100,250,500,1000].forEach(ms=>setTimeout(fix,ms));
  };
  ...
  const apply=()=>{applyPuppyLogo();applyLoginMessage();cleanAccountModal();removeOldAccountText();addHeaderControls();addChangePassword();addPasswordRecovery();patchSameDayTradeSave()};
  apply();
  document.addEventListener('click',e=>{if(e.target.closest?.('#settings'))setTimeout(forceAccountMessage,0);},true);
